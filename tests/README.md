...
```bash
git clone https://github.com/devSviat/BlackBox-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/BlackBox/     ← модуль
└── tests/Modules/Sviat/BlackBox/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/BlackBox
cp -r ~/BlackBox-OkayCMS/tests/. tests/Modules/Sviat/BlackBox/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/BlackBox
```
...
2. Неймспейс — `Modules\Sviat\BlackBox`.
...
use Okay\Modules\Sviat\BlackBox\Services\SomeService;
